...
<div class="max-w-3xl">
    ...
    <div class="bg-white border border-slate-200 rounded-lg p-4 mb-6 flex items-center gap-4">
        ...
        <div>
            <p class="font-semibold text-slate-900"><?= htmlspecialchars($guru['nama']) ?></p>
            <p class="text-sm text-slate-500">Username: <?= htmlspecialchars($guru['username']) ?> · Guru</p>
        </div>
        ...
        <div class="bg-white border border-slate-200 rounded-lg p-5">
            ...
            <div id="gridFoto" class="grid grid-cols-5 gap-1.5">
                ...
                <div id="slot-<?= $i ?>"
                    ...
                    <?php if ($fotoAda): ?>
                    <img src="<?= APP_URL ?>/python/dataset/<?= $guru['username'] ?>/foto_<?= $i ?>.jpg"
            ...
            <div class="mt-4 p-3 bg-green-50 border border-green-200 rounded-md text-sm text-green-800 font-medium">
                ✓ Dataset lengkap. Hubungi Admin untuk melakukan training model ulang agar wajah Anda dapat dikenali.
            </div>
            <?php elseif ($bisaTraining): ?>
            <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md text-sm text-blue-800">
                Dataset cukup (<?= $jumlahFoto ?>/<?= $targetFoto ?>). Silakan hubungi Admin untuk training model, atau lengkapi foto untuk hasil lebih optimal.
            </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="mt-4">
        <a href="<?= APP_URL ?>/absensi-guru"
           class="text-sm text-slate-500 hover:text-slate-700">← Kembali ke dashboard guru</a>
    </div>
</div>
...
<div id="dialogRetake"
    ...
    <div class="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-sm mx-4 p-6">
        <div class="flex items-start gap-3 mb-4">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">
                    Semua <strong id="konfirmasiJumlah"><?= $jumlahFoto ?></strong> foto Anda akan dihapus dari folder dataset. Tindakan ini tidak bisa dibatalkan.
                </p>
            </div>
        </div>
        <div class="flex gap-2 justify-end">
            ...
            <form method="POST" action="<?= APP_URL ?>/absensi-guru/dataset/hapus" class="inline">
...
<script>
window.__GURU_ID__     = <?= (int) $guru['id'] ?>;
...
<script src="<?= APP_URL ?>/public/js/dataset_guru.js?v=<?= filemtime(BASE_PATH . '/public/js/dataset_guru.js') ?>"></script>
